...
def compute_percentage_of_white_pixels(image: Image) -> float:
    """
    ...
    percentage = (white_pixels / (image.width * image.height)) * 100
    return percentage
...
def find_similar_images(userpaths, hashfunc=imagehash.average_hash):
    ...
    for img in sorted(image_filenames):
        try:
            ...
            percentage = compute_percentage_of_white_pixels(img_file)
            hash = hashfunc(img_file, hash_size=8)
            if percentage < 5:
                hash = hashfunc(img_file, hash_size=6)
